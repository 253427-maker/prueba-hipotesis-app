...
with col2:
    ...
    ax2.boxplot(datos, vert=True, patch_artist=True, boxprops=dict(facecolor="steelblue", color="navy"))
...
if tipo_prueba == "Bilateral (≠)":
    ax3.fill_between(x_vals, stats.norm.pdf(x_vals), where=(x_vals <= -z_critico), color="red", alpha=0.4, label="Zona de rechazo")
    ax3.fill_between(x_vals, stats.norm.pdf(x_vals), where=(x_vals >= z_critico), color="red", alpha=0.4)
elif tipo_prueba == "Cola izquierda (<)":
    ax3.fill_between(x_vals, stats.norm.pdf(x_vals), where=(x_vals <= z_critico), color="red", alpha=0.4, label="Zona de rechazo")
else:
    ax3.fill_between(x_vals, stats.norm.pdf(x_vals), where=(x_vals >= z_critico), color="red", alpha=0.4, label="Zona de rechazo")
...
st.session_state.api_key = st.text_input("Ingresa tu API Key de Gemini", value=st.session_state.api_key)
if st.button("Consultar a Gemini"):
    if st.session_state.api_key:
        try:
            prompt = f"""Se realizó una prueba Z:
...
Explica la decisión en términos simples."""
            ...
            st.info(texto)
            st.write(f"**Mi decisión:** {'Rechazar H₀' if rechazar else 'No rechazar H₀'}")
